...
class APGD():
    ...
    """
    def __init__(self, device, predict, loss_fn='ce', n_restarts=2, eps=0.3, nb_iter=40, ord=np.inf, seed=1):
...
class LinfAPGDAttack(APGD):
    ...
    def __init__(self, device, predict, loss_fn='ce', n_restarts=3, eps=0.3, nb_iter=40, seed=1):
        ord = np.inf
        super(LinfAPGDAttack, self).__init__(
            device = device, predict=predict, loss_fn=loss_fn, n_restarts=n_restarts, eps=eps, nb_iter=nb_iter, ord=ord, seed=seed)
